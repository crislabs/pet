...
import { petDeletePages0 } from '@/lib/page/page0/deletePage0';
import { petDeletePages1 } from '@/lib/page/page1/deletePage';
import { petDeleteAdoptions } from '@/lib/product/adoptions/deleteAdoption';
...
export function HeadingDashboardOption() {
  ...
  const { mutate: deletePages0Pet } = useMutation(
    {
      ...
      onSuccess:  (ids) => {
        queryClient.setQueryData<Page[]>(['pet-get-pages0', process.env.NEXT_PUBLIC_SITE_URL as string], (old) => old?.filter(data => !ids.includes(data._id)))
        // queryClient.invalidateQueries(['pet-get-site', process.env.NEXT_PUBLIC_SITE_URL as string])
        Swal.fire({
          title: 'Deleted!',
  ...
  const { mutate: deletePages1Pet } = useMutation(
    {
      mutationFn: async (ids: string[]) => await petDeletePages1(ids),
      onSuccess:  (ids) => {
        queryClient.setQueryData<Page[]>(['pet-get-pages1', query[3]], (old) => old?.filter(data => !ids.includes(data._id)))
        // queryClient.invalidateQueries(['pet-get-site', process.env.NEXT_PUBLIC_SITE_URL as string])
        Swal.fire({
          title: 'Deleted!',
          text: 'Your file has been deleted.',
  ...
  const { mutate: deleteAdoptionsPet } = useMutation(
    {
      mutationFn: async (ids: string[]) => await petDeleteAdoptions(ids),
      onSuccess:  (ids) => {
        queryClient.setQueryData<Product[]>(['pet-get-adoptions', query[3]], (old) => old?.filter(data => !ids.includes(data._id)))
        // queryClient.invalidateQueries(['pet-get-site', process.env.NEXT_PUBLIC_SITE_URL as string])
        Swal.fire({
          title: 'Deleted!',
          text: 'Your file has been deleted.',
